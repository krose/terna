...
#'
#'
terna_energy_balance <- function(date_from, date_to,
  ...
  url <- paste0("https://api.terna.it/transparency/v1.0/getenergybalance?dateFrom=", date_from, "&dateTo=", date_to,
  ...
  cont_df <- tibble::as_tibble( cont_df$energyBalance )

  names(cont_df) <- tolower(names(cont_df))
  names(cont_df)[1] <- "dt"

  cont_df$dt <- lubridate::ymd_hms(cont_df$dt, tz = "Europe/Rome")
  cont_df$energy_balance_gwh <- as.numeric(cont_df$energy_balance_gwh)
